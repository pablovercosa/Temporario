...
Begin VB.Form frmProgramaFidelidadeOperacoesSaida 
   Caption         =   " Programa Fidelidade x Operações Saída"
   ClientHeight    =   6210
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   8790
   ...
   EndProperty
   Icon            =   "frmProgramaFidelidadeOperacoesSaida.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   6210
   ScaleWidth      =   8790
   StartUpPosition =   1  'CenterOwner
   Begin VB.ComboBox cmb_opSaida 
      Appearance      =   0  'Flat
      Enabled         =   0   'False
      Height          =   315
      Left            =   1500
   ...
   Begin VB.CommandButton cmd_novo 
      BackColor       =   &H00C0FFFF&
      Caption         =   "Vincular Operção de Saída"
      Height          =   430
      Left            =   120
      Style           =   1  'Graphical
      TabIndex        =   2
   ...
   Begin VB.Label Label4 
      Caption         =   "Operações de Saída vinculadas ao Programa de Fidelidade"
      Height          =   225
      Left            =   120
      TabIndex        =   8
      Top             =   930
   ...
   End
End
Attribute VB_Name = "frmProgramaFidelidadeOperacoesSaida"
...
Private Sub cmd_cancelar_Click()
  ...
  cmb_opSaida.Enabled = False

  cmb_opSaida.ListIndex = -1
  s_Funcao = ""
End Sub
...
On Error GoTo Erro
  ...
  iCodOperacao = grade_programas.TextMatrix(grade_programas.RowSel, 1)
  
  'db_SQLSERVER.Execute "Delete from [ProgramaFidelidade_OperacaoSaida] where CNPJ = '" & gCNPJ_CPFControleDeLicencaWebApi & "' and [Cd_OperacaoSaida] = " & iCodOperacao

  sSql = "Delete from [ProgramaFidelidade_OperacaoSaida] where CNPJ = '" & gCNPJ_CPFControleDeLicencaWebApi & "' and [Cd_OperacaoSaida] = " & iCodOperacao
...
On Error GoTo Erro:
  ...
  If s_Funcao = "INCLUIR" Then
    ...
    If cmb_opSaida.Text = "" Then
      MsgBox "Informe uma Operação de Saída", vbInformation, "Atenção"
      Exit Sub
    End If

    arrOp = Split(cmb_opSaida.Text, " - ")

    sSql = "Insert into ProgramaFidelidade_OperacaoSaida (CNPJ,Cd_OperacaoSaida) "
...
On Error GoTo Erro:
  ...
  grade_programas.TextMatrix(0, 2) = "Nome Operação de Saída"
  ...
  Dim rsOpSaidas As Recordset
  Set rsOpSaidas = db.OpenRecordset("Select Código, Nome from [Operações Saída]")
  
  While Not rsOpSaidas.EOF
    cmb_opSaida.AddItem rsOpSaidas.Fields(0).Value & " - " & rsOpSaidas.Fields(1).Value
...
On Error GoTo Erro
  ...
  strSQL = "SELECT * FROM ProgramaFidelidade_OperacaoSaida "
  ...
  While Not rsPrograma.EOF
      ...
      grade_programas.AddItem 0 & vbTab & rsPrograma.Fields("Cd_OperacaoSaida").Value
