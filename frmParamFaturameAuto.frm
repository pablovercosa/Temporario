...
Begin VB.Form frmParamFaturameAuto 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Parâmetros para Faturamento Automático"
   ClientHeight    =   3675
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   7815
   ...
   EndProperty
   Icon            =   "frmParamFaturameAuto.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ScaleHeight     =   3675
   ScaleWidth      =   7815
   Begin VB.Frame fraY 
      Caption         =   "Valores Padrões para cada Saída (Serviços)"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   8.25
         Charset         =   0
      ...
      Begin VB.Data datCaixa 
         ...
         Width           =   1140
      End
      Begin VB.TextBox txtNomeServico 
         Appearance      =   0  'Flat
         BackColor       =   &H00808080&
         BeginProperty Font 
            Name            =   "Tahoma"
      ...
      Begin VB.Data datServico 
         ...
         ReadOnly        =   0   'False
         RecordsetType   =   1  'Dynaset
         RecordSource    =   "SELECT Código, Descrição, Preço, ISS FROM Serviços WHERE Código <> 0 ORDER BY Código"
         Top             =   2760
         Visible         =   0   'False
         Width           =   1140
      End
      ...
      Begin VB.Data datOperacao 
         ...
         ReadOnly        =   0   'False
         RecordsetType   =   1  'Dynaset
         RecordSource    =   "SELECT Código, Nome, Tipo FROM [Operações Saída] WHERE Dinheiro AND Nota ORDER BY Código"
         Top             =   2760
         Visible         =   0   'False
         Width           =   1140
      End
      Begin SSDataWidgets_B.SSDBCombo cboOperacao 
         Bindings        =   "frmParamFaturameAuto.frx":058A
         Height          =   315
         Left            =   1200
         TabIndex        =   1
         Top             =   795
      ...
      End
      Begin SSDataWidgets_B.SSDBCombo cboServico 
         Bindings        =   "frmParamFaturameAuto.frx":05A4
         Height          =   315
         Left            =   1200
         TabIndex        =   2
         Top             =   1230
      ...
      Begin SSDataWidgets_B.SSDBCombo cboCaixa 
         Bindings        =   "frmParamFaturameAuto.frx":05BD
         Height          =   315
         Left            =   1200
         TabIndex        =   3
         Top             =   1650
      ...
      Begin SSDataWidgets_B.SSDBCombo cboTabela 
         Bindings        =   "frmParamFaturameAuto.frx":05D4
         Height          =   315
         Left            =   1200
         TabIndex        =   4
         Top             =   2085
      ...
      Begin SSDataWidgets_B.SSDBCombo cboFilial 
         Bindings        =   "frmParamFaturameAuto.frx":05EC
         Height          =   315
         Left            =   1200
         TabIndex        =   0
         Top             =   360
      ...
      Begin ActiveBarLibraryCtl.ActiveBar ActiveBar1 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Bands           =   "frmParamFaturameAuto.frx":0608
      End
   ...
   End
End
Attribute VB_Name = "frmParamFaturameAuto"
...
Private Sub cboServico_CloseUp()
  cboServico.Text = cboServico.Columns(0).Text
  cboServico_LostFocus
End Sub

Private Sub cboServico_LostFocus()
  Dim rstServico As Recordset
  
  txtNomeServico.Text = ""
    
  If Not IsNumeric(cboServico.Text) Then Exit Sub
  
  Set rstServico = db.OpenRecordset("SELECT Código, Descrição FROM Serviços WHERE Código = " & CByte(cboServico.Text), dbOpenSnapshot)
  
  With rstServico
    If Not (.BOF And .EOF) Then
      txtNomeServico.Text = .Fields("Descrição").Value & ""
    End If
    .Close
  End With
...
Private Sub Form_Load()
  ...
  datServico.DatabaseName = gsQuickDBFileName
  datTabela.DatabaseName = gsQuickDBFileName
  datCaixa.DatabaseName = gsQuickDBFileName
  
  Set rstParamFaturameAuto = db.OpenRecordset("SELECT * FROM ParamFaturameAuto ORDER BY Filial", dbOpenDynaset)
  Me.Show
...
Private Function ValidarCampos() As Boolean
  ...
  If Len(txtNomeServico.Text) <= 0 Then
    ValidarCampos = True
    MsgBox "Serviço inválido, verifique.", vbExclamation, "Quick Store"
    cboServico.SetFocus
    Exit Function
  End If
  ...
  If Not IsNumeric(txtISS.Text) Then
    ValidarCampos = True
    MsgBox "Percentual de ISS inválido, verifique.", vbExclamation, "Quick Store"
    txtISS.SetFocus
    Exit Function
  End If
...
Private Sub UpdateRecord()
  ...
  DoEvents
   ...
   With rstParamFaturameAuto
     ...
     .Fields("Servico").Value = CInt(cboServico.Text)
     .Fields("Caixa").Value = CByte(cboCaixa.Text)
     .Fields("Tabela").Value = cboTabela.Text
     .Fields("ISS").Value = Format(txtISS.Text, "##,##0.00")
...
Public Sub ClearScreen()
  ...
  If Not rstParamFaturameAuto.EOF Then
    On Error Resume Next
    rstParamFaturameAuto.MoveFirst
    rstParamFaturameAuto.MovePrevious
    On Error GoTo 0
  End If
...
Private Sub VerificarSeExisteRegistro()
  ...
  strQuery = "SELECT * FROM ParamFaturameAuto "
  strQuery = strQuery & " WHERE Filial = " & CByte(cboFilial.Text)
  
  Set rstParamFaturameAuto = db.OpenRecordset(strQuery, dbOpenDynaset)
  
  If rstParamFaturameAuto.RecordCount = 0 Then Exit Sub
